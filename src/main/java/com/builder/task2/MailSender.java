...
public class MailSender {
    ...
    public MailSender() {
        ...
        if (apiKey.isEmpty() || apiSecret.isEmpty()) {
            throw new IllegalArgumentException("API Key or Secret is missing");
        }
    ...
    public void sendMail(MailInfo mailInfo) {
        try {
            ...
            emailData.put(Emailv31.Message.TO, new JSONArray()
                    .put(new JSONObject()
                            .put("Email", "ira.kirillova0806@gmail.com")
            ...
            emailData.put(Emailv31.Message.HTMLPART, "<h3>" + mailContent + "</h3>");

            JSONArray messages = new JSONArray();
            messages.put(emailData);


            MailjetResponse response = client.post(new MailjetRequest(Emailv31.resource)
                    .property(Emailv31.MESSAGES, messages));

            if (response.getStatus() == 200) {
                System.out.println("Email sent successfully");
            } else {
                System.out.println("Failed to send email. Response: " + response.getStatus());
            }
